...
describe.skip("Calendar page", () => {
  ...
  let sessionHeaderBuilder = new XLinkHeaderBuilder().value(
    new SessionXLinkValueHeaderBuilder().current(
      new Date("2021-11-01T00:00:00")
    )
  );
  ...
  beforeEach(() =>
    sessionStorage.setItem(
      "token",
      JSON.stringify({ token: "my-token", type: "bearer" })
    )
  );
  ...
  it("should be displayed", async () => {
    ...
    expect(
      await waitFor(() => screen.findByText(/monday/i))
    ).toBeInTheDocument();
    expect(
      await waitFor(() => screen.findByText(/tuesday/i))
    ).toBeInTheDocument();
    expect(
      await waitFor(() => screen.findByText(/wednesday/i))
    ).toBeInTheDocument();
    expect(
      await waitFor(() => screen.findByText(/thursday/i))
    ).toBeInTheDocument();
    expect(
      await waitFor(() => screen.findByText(/friday/i))
    ).toBeInTheDocument();
    expect(
      await waitFor(() => screen.findByText(/saturday/i))
    ).toBeInTheDocument();
  });
});
