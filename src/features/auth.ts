...
const getToken = (token: Token): Token => {
  const retrieveToken = (token: string): Token => {
    ...
    const isTokenExpirationInTheFuture =
      compareAsc(fromUnixTime(exp), new Date()) >= 0;
    return isTokenExpirationInTheFuture
...
>("login", async (login, thunkAPI) => {
  try {
    ...
    return thunkAPI.rejectWithValue(
      map_action_thunk_error("Authentication failed", e as ApiError)
    );
  }
});
