...
describe.skip("ClientList page", function () {
  describe("faces an error when creating a client", function () {
    ...
    it("should display the error", async () => {
        ...
        new RequestHandlerBuilders()
          ...
          .body(
            new APIErrorBody()
              .withDetail(
                APIDetail(
                  "You must provide the client firstname",
                  "value_error"
                )
              )
              .withDetail(
                APIDetail("You must provide the client lastname", "value_error")
              )
              .build()
          )
          .build()
      ...
      await screen.findByText("You must provide the client firstname", {
        selector: "p",
      });
      await screen.findByText("You must provide the client lastname", {
        selector: "p",
      });
