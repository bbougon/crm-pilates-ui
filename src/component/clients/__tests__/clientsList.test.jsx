  ...
  describe("displays a form to create a client", function () {
    ...
    describe("with credits", () => {
      it("named Joseph Pilates", async () => {
        ...
        expect(
          await screen.findByText("Pilates", { selector: "h6" })
        ).toBeInTheDocument();
        ...
        userEvent.click(
          await screen.findByRole("button", { name: /pilates/i })
        );

        const clientDetails = screen.getAllByRole("region")[1];
        expect(
          within(clientDetails).getByText("10", { selector: "span" })
        ).toBeInTheDocument();
        expect(
          within(clientDetails).getByText(/machine trio/i, { selector: "p" })
        ).toBeInTheDocument();
        expect(
          within(clientDetails).getByText("6", { selector: "span" })
        ).toBeInTheDocument();
        expect(
          within(clientDetails).getByText(/machine duo/i, { selector: "p" })
        ).toBeInTheDocument();
