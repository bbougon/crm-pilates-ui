...
export function schedulingReducer(
  ...
  const durationAndStartDate = ({
    hours,
    minutes,
    key,
    date,
  }: ScheduleInformation): SchedulingState => {
    return determineClassroomTimeAndDuration({ hours, minutes, key, date });
  };
  const durationAndEndDate = ({
    hours,
    minutes,
    key,
    date,
  ...
  const determineClassroomTimeAndDuration = ({
    hours,
    minutes,
    key,
    date,
    ...
    if (hours) {
      ...
      const foundAvailableDuration = state.availableDurations.find(
        (availableDuration) => availableDuration.duration === duration
      );
  ...
  switch (action.type) {
    ...
    case ActionType.DURATION_UPDATED: {
      ...
      const endDateTimeAtSameTimeThanStartDateTime = set(
        parseISO(state.classroomEndDateTime),
        {
          hours: startDate.getHours(),
          minutes: startDate.getMinutes(),
        }
      );
      const classroomEndDateTime = formatISO(
        addMinutes(endDateTimeAtSameTimeThanStartDateTime, action.duration)
      );
    ...
    case ActionType.ATTENDEES_UPDATED:
      return {
        ...
        position:
          action.attendees.length > state.position
    ...
    case ActionType.START_DATE_UPDATED: {
      ...
      return durationAndStartDate({
        hours,
        minutes,
        key: "classroomStartDateTime",
    ...
    case ActionType.END_DATE_UPDATED: {
      ...
      return durationAndEndDate({
        hours,
        minutes,
        key: "classroomEndDateTime",
        date: action.endDate,
      });
    }
  }
