...
abstract class XLinkValueHeaderBuilder implements ValueHeaderBuilder {
  ...
  build = (): string => {
    return (
      this.value
        ?.map((xlink) => xlink.link.concat("; rel=").concat(xlink.rel))
        .join(", ") || ""
    );
  };
}
...
export class SessionXLinkValueHeaderBuilder extends XLinkValueHeaderBuilder {
  ...
  current = (currentMonth: Date): SessionXLinkValueHeaderBuilder => {
    ...
    this.value = [
      ...
      {
        link: `</sessions?month=${currentMonth}>` as Link,
        rel: "current" as Rel,
      },
...
abstract class RequestHandlerBuilder<TBody> {
  ...
  searchParams = <TSearchParams extends object>(
    params: TSearchParams
  ): RequestHandlerBuilder<TSearchParams> => {
    this._searchParams = params;
    return this;
  };

  header = <T extends HeaderBuilder>(
    header: T
  ): RequestHandlerBuilder<TBody> => {
...
export class GetRequestHandlerBuilder<
  TBody
> extends RequestHandlerBuilder<TBody> {
  build(): RequestHandler {
    return rest.get(API_URI + this.path, (req, res, _) => {
      ...
      if (this._searchParams !== null) {
        ...
        if (
          Object.entries(req.url.searchParams).toString() !==
          Object.entries(this._searchParams).toString()
        ) {
          statusCode = 400;
        }
